...
for(const token of ['calculadoras-mx-v3.10.1','/offline.html','skipWaiting','clients.claim'])if(!sw.includes(token))errors.push(`Service worker sin ${token}`);
...
for(const token of ['CM_CATALOG_BUNDLE_START','CM_CALCULATORS_BUNDLE_START','CM_SIMULATORS_BUNDLE_START'])if(!common.includes(token))errors.push(`Bundle común sin ${token}`);
