...
// V4.1 quality gate: every calculator must contain a substantive guide and at least five FAQs.
for (const name of calculators) {
  const html = fs.readFileSync(path.join(root,'calculadoras',name),'utf8');
  const guide = html.match(/<section class="content-box seo-guide">([\s\S]*?)<\/section>/)?.[1] || '';
  const plainGuide = guide.replace(/<[^>]+>/g,' ').replace(/\s+/g,' ').trim();
  const faqCount = (html.match(/<summary>/g) || []).length;
  if (plainGuide.length < 500) errors.push(`calculadoras/${name}: guía propia demasiado corta (${plainGuide.length} caracteres)`);
  if (faqCount < 5) errors.push(`calculadoras/${name}: menos de cinco preguntas frecuentes`);
}
