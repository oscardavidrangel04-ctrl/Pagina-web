...
for (const calc of calculators) {
  ...
  html = html.replace('Portal V3 — Fase 4','Cálculo claro y privado');
...

{
  ...
  let html = fs.readFileSync(target,'utf8');
  html = html.replace('<span class="badge">Fase 4</span>','<span class="badge">Nuevas</span>');
...
for (const file of fs.readdirSync(path.join(root,'articulos')).filter(name=>name.endsWith('.html'))) {
  ...
  const articleSchema = JSON.stringify({'@context':'https://schema.org','@type':'BlogPosting','headline':title,'description':description,'image':`${site}/assets/img/og-image.png`,'author':{'@type':'Organization','name':'Calculadoras México'},'publisher':{'@type':'Organization','name':'Calculadoras México','logo':{'@type':'ImageObject','url':`${site}/assets/img/logo.svg`}},'mainEntityOfPage':`${site}/articulos/${file}`,'dateModified':'2026-08-01'});
...
const sitemap = `<?xml version="1.0" encoding="UTF-8"?>\n<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">\n${urls.map(url => `  <url><loc>${site}/${url}</loc><lastmod>2026-08-01</lastmod></url>`).join('\n')}\n</urlset>\n`;
