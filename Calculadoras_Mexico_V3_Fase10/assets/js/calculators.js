  ...
  button?.addEventListener('click', () => {
    if (type === 'isr') {
      ...
      return showRows([
        ...
        {label: 'Límite inferior del rango', value: money(row.limiteInferior)},
        ...
        {label: 'Tasa efectiva estimada', value: `${effectiveRate.toFixed(2)}%`},
        {label: 'Neto estimado', value: money(income - tax), total: true}
      ], 'Tarifa del periodo seleccionado. No considera subsidio al empleo, IMSS ni otras deducciones o ajustes de nómina.');
    }
    if (type === 'aguinaldo') {
      const salary = val('salario'), days = val('diasAguinaldo');
      const salaryInfo = globalThis.CMAguinaldo?.getSalaryInfo?.() || {period:'mensual', label:'Mensual', amount:salary, divisor:30, daily:salary/30};
      const scenario = globalThis.CMAguinaldo?.getScenario?.() || {mode: 'dias', days: val('diasTrabajados'), label: 'Días capturados'};
      if (salary <= 0 || days < 15) return fail('Ingresa un salario mayor que cero y al menos 15 días de aguinaldo');
      if (!Number.isFinite(salaryInfo.daily) || salaryInfo.daily <= 0) return fail('No se pudo obtener una referencia diaria válida');
      if (scenario.error) return fail(scenario.error);
      if (!Number.isFinite(scenario.days) || scenario.days <= 0 || scenario.days > 365) return fail('Los días considerados deben estar entre 1 y 365');
      const worked = Math.min(scenario.days, 365);
      const daily = salaryInfo.daily;
      const full = daily * days;
      const minimumFull = daily * 15;
      const proportion = worked / 365;
      const proportional = full * proportion;
      const minimumProportional = minimumFull * proportion;
      const difference = proportional - minimumProportional;
      const equivalentDays = days * proportion;
      const periodRow = scenario.mode === 'fechas' && scenario.period ? [{label: 'Periodo considerado', value: scenario.period}] : [];
      const rows = [
        {label: 'Escenario', value: scenario.label || 'Aguinaldo proporcional'},
        ...periodRow,
        {label: `Salario ${salaryInfo.label.toLowerCase()} capturado`, value: money(salary)},
        {label: 'Salario diario de referencia', value: money(daily)},
        {label: 'Días de aguinaldo anual', value: `${days.toFixed(days % 1 ? 2 : 0)} días`},
        {label: 'Días considerados en 2026', value: `${worked.toFixed(0)} de 365`},
        {label: 'Proporción del año', value: `${(proportion * 100).toFixed(2)}%`},
        {label: 'Días equivalentes de aguinaldo', value: `${equivalentDays.toFixed(2)} días`},
        {label: 'Aguinaldo anual de referencia', value: money(full)},
        {label: worked === 365 ? 'Aguinaldo estimado' : 'Aguinaldo proporcional estimado', value: money(proportional), total: true}
      ];
      showRows(rows, `Conversión salarial: ${money(salary)} ÷ ${salaryInfo.divisor} = ${money(daily)} diarios. Fórmula: ${money(daily)} × ${days} días × (${worked}/365). Estimación bruta; no descuenta ISR ni otros ajustes de nómina.`);
      globalThis.CMAguinaldo?.renderGuide?.({
        mode: scenario.mode,
        period: scenario.period,
        worked,
        daily: money(daily),
        days: days.toFixed(days % 1 ? 2 : 0),
        full: money(full),
        percentage: (proportion * 100).toFixed(2),
        proportional: money(proportional),
        salaryPeriod: salaryInfo.label,
        salaryAmount: money(salary)
      });
      globalThis.CMAguinaldo?.renderComparison?.({
        days: days.toFixed(days % 1 ? 2 : 0),
        minimum: money(minimumProportional),
        current: money(proportional),
        currentValue: proportional,
        difference: money(Math.abs(difference)),
        differenceValue: difference,
        dailyValue: daily,
        proportion,
        worked: worked.toFixed(0),
        percentage: (proportion * 100).toFixed(2)
      });
      globalThis.CMAguinaldo?.setLastResult?.({salaryPeriod: salaryInfo.label, salaryAmount: salary, annualDays: days, mode: scenario.mode, period: scenario.period || ''});
      return;
    }
    if (type === 'vacaciones') {
      ...
      const days = years <= 5 ? 10 + years * 2 : 22 + Math.floor((years - 6) / 5) * 2;
